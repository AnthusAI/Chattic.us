...
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  return buildStaticParams("updates");
}

export async function generateMetadata({ params }: UpdatesPostPageProps): Promise<Metadata> {
  const { slug } = await params;
  if (isStaticExportPlaceholder(slug)) {
    notFound();
  }
  const post = getPost("updates", slug);
  if (!post || post.frontmatter.draft) {
    return {};
  }

  const canonical = postPath("updates", slug);

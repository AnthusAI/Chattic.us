...
That is the opposite of a **per-agent laptop** (each bot gets an isolated VM that looks like a personal computer) and the opposite of **computer-use as a tool call** with no disk behind it.
...
Sharing also forces the product to be an [agent workplace](/wiki/agent-workplace), not a coding session. A session can discard the disk. A workplace cannot.

Neighboring ideas: [named teammates](/wiki/named-teammates) sit at the computer; [farms and desks](/wiki/farms-and-desks) describe how the roster uses it; [always-on](/wiki/always-on) is what happens when the computer is *not* the point and the agent simply waits.

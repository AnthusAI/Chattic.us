...
/**
 * Shared Tailwind design tokens (colors, fonts, radius, shadow, keyframes,
 * animation) for the Chatticus brand system. Consumed both by this app and
 * by the private marketing/SaaS repo, which depends on this package and
 * composes this as a `presets` entry with its own `content` glob -- see
 * ./tailwind.config.ts for the app-local usage.
 */
const chatticusTailwindPreset: Pick<Config, "theme" | "plugins"> = {
  theme: {
    extend: {
      colors: {
        paper: "var(--paper)",
...
export default chatticusTailwindPreset;
